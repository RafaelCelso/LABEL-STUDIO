...
import { getAuthClientErrorMessage } from "@/lib/auth/client-error-message";
...
interface SignInComponentProps {
  ...
  /** Recebe o e-mail do passo atual para fluxo de redefinição (ex.: link com query). */
...
export const SignInComponent = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const run = onSignIn ?? signInWithNeonAuth;
    ...
    <div className="bg-background min-h-screen w-screen flex flex-col">
      ...
      >
        <div className="absolute inset-0 z-0">
          <AuthGradientBackground idPrefix="signin" />
        </div>
        <fieldset
              ...
              <BlurFade delay={0.25 * 6} className="w-full text-center">
                <p className="text-sm text-muted-foreground">
                  ...
                  <button
                    type="button"
                    onClick={onSignUp}
                    className="cursor-pointer text-foreground font-semibold hover:underline transition-all"
                  >
                    Cadastre-se
                  </button>
                </p>
